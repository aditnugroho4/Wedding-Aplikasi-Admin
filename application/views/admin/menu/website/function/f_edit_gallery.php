...
String.prototype.replaceAll = function(str1, str2, ignore) {
	...
	$("#tblData").dataTable({
	    ...
	    "sAjaxSource": "<?php echo site_url('admin/get_data_table_source'); ?>?table=w_post_images&columns=,w_post_images.name,w_post_images.date,w_post_kategory.name,size,ctgr_id&jwhere=ctgr_id&fildjoins=,w_post_kategory.name as katagory,w_post_images.name as nama&joins=w_post_kategory&exports=w_post_kategory",
	    "aoColumns": [{
	            "sTitle": "#",
	            "mData": "no",
	            "bSortable": false,
	        ...
	        },
	        {
	            ...
	            "fnCreatedCell": function (nTd, sData, oData, iRow, iCol) {
	                ...
	                $(nTd).attr("id", id);
	                if (oData.img == null) {
	                    htmlx = "<div class='from-group'><img class='img-thumbnail' width='65' src='<?php echo base_url('asset/admin/dist/default-150x150.png'); ?>'alt='AKbar Grup'></div>";
	                    $(nTd).html(htmlx);
	                } else {
						var sizeinbytes =getImageSizeInBytes("<?php echo base_url(); ?>/" + oData.img);
	                    ...
	                    htmlx = "<div id='img-"+oData.id+"' class='from-group'><img class='img-thumbnail'width='65' src='<?php echo base_url(); ?>/" +
	                        oData.img + "' alt='"+oData.nama+"'><span class='ml-5'>Size: ("+ hasil +")</span></div>";
	                    $(nTd).html(htmlx);
	                }
	            }
	        },
	...
	function get_kategory() {
		try {
			...
			$.ajax({
				...
				success: function (data) {
					if (data == '') {
						element.append('<a class="btn btn-info p-1 active" href="javascript:void(0)" data-filter="all"> Data Not Found </a>');
					} else {
						element.append('<a class="btn btn-info p-1 active" href="javascript:void(0)" data-filter="all"> All items </a>');
						for (var i = 0; i < data.length; i++) {
							element.append('<a class="btn btn-info p-1" href="javascript:void(0)" data-filter="' + data[i].id + '"> ' + data[i].name + '</a>');
						}
						get_gallery();
					}
				}
		...
		try {
			...
			$.ajax({
				...
				success: function (data) {
					if (data != "") {
						for (var i = 0; i < data.length; i++) {
							element.find('.list-images').append('<div class="filtr-item col-4 img-lg" data-category="' + data[i].ctgr_id + '" data-sort="' + data[i].id + '"><a href="' + '<?php echo base_url();?>' + data[i].img + '" data-toggle="lightbox" data-title="' + data[i].name + '"><img src="' + '<?php echo base_url(); ?>' + data[i].img + '" class="img-fluid " alt="' + data[i].name + '"/></a></div>');
						}
